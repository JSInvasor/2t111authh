...
const { assess: assessRisk } = require('./risk');
...
/** Device token / environment fingerprint: hex from the loader, or nothing. */
...
/**
 * Does this request come from the device the key is bound to?
 *
 * Two independent identifiers, and DEVICE_MATCH_REQUIRED says how many must
 * agree. The client id on its own is weak — it has public spoofers, and it is
 * one value to fake. The device token is generated by the loader and kept in the
 * executor's own filesystem, so it survives a spoofed client id, but it is lost
 * if the user wipes their workspace.
 *
 * Requiring either one (the default) means a legitimate user who reinstalled
 * their executor keeps working while a spoofed client id alone no longer opens
 * the key. Requiring both is stricter and locks out that reinstall — worth it
 * only where the support cost is acceptable.
 */
function deviceMatches(bound, { hwid, device }) {
  const hwidOk = !!(bound.hwid && hwid && bound.hwid === hwid);
  const tokenOk = !!(bound.device_token && device && bound.device_token === device);

  // Strict: both identifiers must be present and agree. Locks out a user who
  // wiped their executor workspace, so it is opt-in.
  if (config.deviceMatchRequired >= 2) return hwidOk && tokenOk;

  // A token that DISAGREES is not the same as one that is missing, and the
  // difference is the whole point. Missing means "this device can't tell us" —
  // a wiped workspace, an executor without file APIs — and we fall back to the
  // client id. Disagreeing means something positively claims to be a different
  // machine, and since the client id is the spoofable half of the pair, a
  // conflicting token has to outweigh a matching HWID. Otherwise the spoofers
  // that made the client id worth distrusting would still walk straight in.
  if (bound.device_token && device && !tokenOk) return false;

  return hwidOk || tokenOk;
}
 ...
 */
function authenticate({ scriptId, key, hwid, ip, executor, device = null, env = null, session = null }) {
  const exec = cleanExecutor(executor);
  const ctx = { scriptId, hwid, ip, executor: exec };
  device = cleanToken(device);
  env = cleanToken(env);
  ...
  if (script.hwid_lock) {
    ...
    if (!row.hwid) {
      ...
      const bound = db
        .prepare('UPDATE keys SET hwid = ?, device_token = ? WHERE id = ? AND hwid IS NULL')
        .run(hwid, device || null, row.id);
      if (bound.changes === 0) {
        const current = db.prepare('SELECT hwid, device_token FROM keys WHERE id = ?').get(row.id);
        if (!current || !deviceMatches(current, { hwid, device })) {
    ...
    } else if (!deviceMatches(row, { hwid, device })) {
      logExecution({ ...ctx, keyId: row.id, success: false, reason: 'hwid_mismatch' });
      return { success: false, code: 403, message: 'HWID mismatch — this key is locked to another device' };
    } else if (device && !row.device_token) {
      // Same device, first time it has reported a persisted token — record it so
      // the next client-id spoof has something to fail against.
      db.prepare('UPDATE keys SET device_token = ? WHERE id = ? AND device_token IS NULL').run(device, row.id);
    }
  }
  ...
  // Trust on first use: pin what this machine looked like the first time the key
  // worked, and notice when it stops looking like that. The client reports this,
  // so it is a signal and not a gate — it feeds the risk score, it does not
  // decide anything on its own.
  if (config.envPinning && env) {
    if (!row.env_fp) {
      db.prepare('UPDATE keys SET env_fp = ? WHERE id = ? AND env_fp IS NULL').run(env, row.id);
    } else if (row.env_fp !== env) {
      db.prepare('UPDATE keys SET env_fp = ?, env_fp_changes = env_fp_changes + 1 WHERE id = ?').run(env, row.id);
      logExecution({ ...ctx, keyId: row.id, success: false, reason: 'env_changed' });
    }
  }
  ...
  // Weigh everything together. One odd signal is noise — a phone that changed
  // network, a reinstall, a second device the user owns. Several at once is a
  // pattern, and only that gets acted on.
  const risk = assessRisk(row.id, { hwid, ip, env });
  if (risk.verdict === 'ban') {
    logExecution({ ...ctx, keyId: row.id, success: false, reason: `risk_ban:${risk.score}` });
    return { success: false, code: 403, message: 'Key banned — suspicious activity' };
  }
  if (risk.verdict === 'watch') {
    logExecution({ ...ctx, keyId: row.id, success: false, reason: `risk_watch:${risk.score}` });
  }
 ...
 * fresh nonce, spends it once, straight away, from the same network, carrying an
 * HMAC only the license key produces. Every reason routed here is therefore
 ...
 *
 * The key is named by hash, like everywhere else on the public path — the value
 * never travels, so there is nothing here to look it up by.
 *
 * @param {{scriptId:string, kh?:string|null, hwid?:string|null, ip?:string|null,
 *          executor?:string|null, reason:string}} args
 * @returns {{banned:boolean, count:number}}
 */
function recordViolation({ scriptId, kh, hwid, ip, executor, reason }) {
  const row = kh ? db.prepare('SELECT id FROM keys WHERE kh = ? AND script_id = ?').get(String(kh), scriptId) : null;
  const tag = `protocol:${
    String(reason || 'unknown')
      .replace(/[^a-z0-9:_-]/gi, '')
      .slice(0, 32) || 'unknown'
