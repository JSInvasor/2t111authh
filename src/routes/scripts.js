...
router.post('/:id/trace', (req, res) => {
  ...
  if (!config.watermark) {
    return res
      .status(400)
      .json({ success: false, message: 'Watermarking is disabled (WATERMARK=0), so deliveries carry no mark.' });
  }
  if (!script.obfuscate) {
    return res.status(400).json({
      success: false,
      message:
        'This script is delivered unprotected, so its copies are identical and carry no mark. Turn on obfuscation to make future deliveries traceable.',
    });
  }
  ...
  if (!plan) {
    return res.status(400).json({
      success: false,
      message:
        'This script could not be watermarked (its source does not parse as Lua 5.x), so its deliveries carry no mark.',
    });
  }
  ...
  if (!result.carriers_read && /["'][A-Za-z0-9+/=]{200,}["']/.test(sample)) {
    result.note =
      'This looks like an encrypted delivery stub, not the script inside it. Paste the decrypted source that was dumped.';
  }
